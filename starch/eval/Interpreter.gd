...
func eval_function_call(node: ASTFunctionCall):
	...
	if callee is ASTIdentifier:
		...
		# NEW: Check if it's a variable containing a callable first
		if current_env.has(func_name):
			var potential_func = current_env.get_var(func_name)
			if potential_func is Callable:
				var args = []
